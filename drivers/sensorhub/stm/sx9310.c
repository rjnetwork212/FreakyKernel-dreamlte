...
static irqreturn_t sx9310_reinit_int_th(int irq, void *pdata)
{
	...
	if (sx9310_get_nirqldo_state(data) == 1) {
		...
		queue_delayed_work(system_power_efficient_wq, &data->reinit_irq_work, msecs_to_jiffies(10));
	} else {
...
static void sx9310_init_work_func(struct work_struct *work)
{
	...
	if (gpio_get_value_cansleep(data->gpio_ldoen)) {
		...
		if (data->retry_cnt < 10) {
			data->retry_cnt++;
			queue_delayed_work(system_power_efficient_wq, &data->init_work,
		...
		const struct i2c_device_id *id)
{
	...
	queue_delayed_work(system_power_efficient_wq, &data->init_work, msecs_to_jiffies(5000));
#endif
